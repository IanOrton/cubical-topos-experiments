...
pathInv (P , refl , refl) ρ i = subst (λ P' → (Fib P' → IdU (P i) (P' i))) eqTypes result (snd (pathToPath (P , refl , refl) ρ)) where
  ...
  hPath i p' = (path , pathO≡hp' , pathI≡b) where
    ...
    pathO≡hp' = fillAtEnd {A = P} I' ρ (λ j → max i j) (Φ i) (q' i p') (b i p') (q'I↗b i p')
      -- proof:
      --   fst filler O  
  ...
  ghInv p' = (path , pathO≡ghp' , pathI≡p') where
    ...
    qij i j p' = OI-elim' (qi i p') (qj i j p') (corners p') where
      ...
      corners {i} p' e I' i≡e refl = fst (snd (hFiller i p')) ∣ OIinj i e i≡e ∣
      corners p' O' O' refl refl =
        proof:
    ...
    extends j u = OI-elim-dep {B = λ x → f' i x (c i p' x) ≡ pathB p' j} (cases p') u where
      ...
      cases p' (inl refl) = snd (pathBRaw p' j) ∣ inl ∣ inl refl ∣ ∣
      cases p' (inr refl) = snd (pathBRaw p' j) ∣ inl ∣ inr refl ∣ ∣
    path : Int → P' i
  ...
  result ρ' = fst (equivToId {α = ρi} {β = ρ'i} (g i) (qinv2equivSimple ρi ρ'i (g i) (h i , ghInv , hgInv))) where
    ρi : Fib (λ _ → P i)
    ρi e p = ρ e (λ _ → i)
    ρ'i : Fib (λ _ → P' i)
    ρ'i e p = ρ' e (λ _ → i)
